...
// ==========================================================
// 🚨 PERUBAHAN UTAMA DI SINI: upload.array('images', 5)
// Mengganti upload.single menjadi upload.array untuk menerima hingga 5 file
// Nama field harus 'images'
// ==========================================================
...
// ==========================================================
// MENGGANTI: upload.single('image') menjadi upload.array('images', 5)
app.post('/api/ocr', upload.array('images', 5), async (req, res) => {
  const GEMINI_API_KEY = process.env.GEMINI_API_KEY;
  
  // 🚨 PERUBAHAN DI SINI: Menggunakan req.files (array)
  if (!req.files || req.files.length === 0) {
    return res.status(400).json({ error: 'Minimal satu file gambar tidak ditemukan' });
  }
...
- Gunakan bahasa netral dan analitis dalam semua laporan visual.
  ...
  // 🚨 PERUBAHAN DI SINI: Membuat array 'parts' yang berisi prompt teks dan SEMUA file gambar
  const parts = [{ text: abidinaiPrompt }];

  // Tambahkan setiap file gambar ke array parts
  req.files.forEach((file, index) => {
    ...
    console.log(`Menambahkan Gambar ${index + 1} (${file.mimetype}) ke payload.`);
  });
  ...
  const payload = {
    contents: [
      {
        parts: parts // Mengirim semua prompt dan gambar sekaligus
      }
    ]
  };
